...
export const SPEAKING_RULES: RuleGroup = {
  ...
  rules: [
    ...
    'After one substantive student response + brief feedback: complete and move on, except reason/example speaking tasks may run a bounded mini-dialogue before completing',
    'A one-word, filler, or clearly incomplete response is NOT substantive — ask once for a fuller answer',
    'If the task asks for reasons, an opinion fragment without a reason must be scaffolded for one clear reason or one reason plus example — do not repeat the whole prompt',
