...
acceptance criterion in `.claude/GLOBAL_GOAL.md`.
...
| File | Purpose |
|------|---------|
| `.claude/GLOBAL_GOAL.md` | Source of truth — acceptance criteria |
| `.claude/GOAL_PROGRESS.md` | History to check for gaps and false claims |
| `.claude/NEXT_ACTION.md` | Claimed current state |
| `.claude/RISK_REGISTER.md` | Accepted risks that may affect criteria |
| `.claude/DECISIONS.md` | Architectural decisions that may affect scope |
...
Read `.claude/GLOBAL_GOAL.md` in full. List every acceptance criterion
...
- Do write `.claude/REVIEW_REPORT.md` → Acceptance Auditor Verdict section.
...
1. **Append to `.claude/REVIEW_REPORT.md`** the full Acceptance Auditor Verdict section.

2. **If GOAL NOT COMPLETE** — write `.claude/NEXT_ACTION.md` with the highest-priority
   remaining task from the Revised Roadmap. Use the NEXT_ACTION template format.

3. **If GOAL COMPLETE** — write to `.claude/NEXT_ACTION.md`:
   ```
